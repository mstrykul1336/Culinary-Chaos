...
public class Leaderboard : MonoBehaviour
{
    ...
    void UpdateLeaderboardUI(List<PlayerLeaderboardEntry> leaderboard)
    {
        for (int x = 0; x < leaderboardEntries.Length; x++)
        {
            ...
            leaderboardEntries[x].transform.Find("PlayerText").GetComponent<TextMeshProUGUI>().text = (leaderboard[x].Position + 1) + ". " + leaderboard[x].DisplayName;
            leaderboardEntries[x].transform.Find("ScoreText").GetComponent<TextMeshProUGUI>().text = (-(float)leaderboard[x].StatValue).ToString("F2");
        }
    }
            ...
            PlayFabClientAPI.UpdatePlayerStatistics(new UpdatePlayerStatisticsRequest
                {
                    ...
                    {
                        new StatisticUpdate { StatisticName = "MostKills", Value = newScore },
                    }
                },
